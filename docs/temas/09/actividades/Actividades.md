# **UT9: Actividades — Ficheros en Java**

!!! info "Organización de las actividades"

    Las actividades están organizadas en **cuatro niveles de dificultad progresiva**. Es importante realizarlas en orden, ya que cada nivel construye sobre los conocimientos del anterior.

    | Nivel | Actividades | Contenidos que trabaja |
    |-------|-------------|------------------------|
    | 🟢 **Básico** | Act01 – Act04 | Operaciones con ficheros/directorios, lectura y escritura simple |
    | 🟡 **Intermedio** | Act05 – Act07 | try-with-resources, procesamiento CSV, Stream básico |
    | 🔴 **Avanzado** | Act08 – Act10 | Collectors, Files.walk(), serialización |
    | 🏆 **Integrador** | Act11 | Proyecto completo: aplicación de gestión con ficheros |

    **Criterios de evaluación que se trabajan:** CE5a · CE5b · CE5c · CE5d · CE5e

---

## Estructura del Proyecto

El código base se encuentra en la carpeta [`ejercicios-ficheros`](./ejercicios-ficheros.zip). Debes implementar la lógica en las clases dentro de `src/main/java`.
...
**Objetivo:** Practicar la creación de directorios y ficheros con `Files.createDirectories()` y `Files.createFile()`, y verificar su existencia con `Files.exists()`.
**Criterios:** CE5c · CE5d
...
??? question "Pistas"
    ...
    - `Files.exists(path)` devuelve `true` si ya existe.
    - `Files.createDirectories(path)` crea también los directorios intermedios.
    - Declara dos contadores (`creadosDirs`, `creadosFichs`) e increméntalos cuando se crea algo.
...
**Extensión opcional:** Añade un método `limpiarEspacio()` que elimine todos los ficheros (no los directorios) usando `Files.deleteIfExists()`, mostrando cuántos se han eliminado.
...
**Objetivo:** Practicar `Files.readAllLines()`, recorrido de listas, formateo de salida y búsqueda básica en texto.
**Criterios:** CE5a · CE5b · CE5d
...
??? question "Pistas"
    ...
    - `String.toLowerCase().contains(palabra.toLowerCase())` para búsqueda insensible a mayúsculas.
    - `String.format("%3d", numLinea)` para alinear los números.
    - `List.size()` para contar el total de líneas.
...
**Objetivo:** Practicar `Files.write()` con `StandardOpenOption.APPEND`, formateo de fechas y hora con `LocalDateTime`, y lectura con `readAllLines`.
**Criterios:** CE5a · CE5b · CE5d
...
??? question "Pistas"
    ...
    - `DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")` y `LocalDateTime.now()`.
    - `String.format("[%-8s]", categoria)` rellena con espacios hasta 8 caracteres a la izquierda.
    - Para `contarPorCategoria`: recorre las líneas y usa un `Map<String, Integer>` para contar.
...
**Objetivo:** Practicar `DirectoryStream`, `Files.readAttributes()`, `Files.copy()`, `Files.move()` y `Files.delete()` de forma integrada.
**Criterios:** CE5c · CE5d · CE5e
...
??? question "Pistas"
    ...
    - `attrs.size() / 1024.0` para obtener KB con decimales.
    - `attrs.lastModifiedTime().toInstant()` + `ZoneId.systemDefault()` + `DateTimeFormatter`.
    - Para glob: `Files.newDirectoryStream(dir, "*.tmp")`.
...
**Objetivo:** Dominar el patrón `try-with-resources` con múltiples recursos, `BufferedReader`/`BufferedWriter` y entender el cierre automático.
**Criterios:** CE5c · CE5d
...
??? question "Pistas"
    ...
    - `try (BufferedReader r = ...; BufferedWriter w = ...) { ... }` — ambos en el mismo try.
    - Para la máscara del email: `email.substring(email.indexOf('@'))` te da `@dominio.com`, y luego concatenas `"***"`.
    - `String.split("\\s+")` para contar palabras (separa por uno o más espacios).
    - `BufferedWriter` tiene el método `newLine()` para escribir el salto de línea del sistema.
...
**Extensión opcional:** Añade un método `descifrarBackup(Path fichero)` que lea el fichero línea a línea con `BufferedReader` y elimine el prefijo que añadiste, restaurando el original.
...
**Objetivo:** Dominar el procesamiento completo de ficheros CSV: lectura, parsing, validación línea a línea y generación de informes.
**Criterios:** CE5b · CE5d · CE5e
...
??? question "Pistas"
    ...
    - `campos[3].trim().isEmpty()` para detectar notas vacías.
    - `String.format("%-28s %5s %6s %6s %7s  %-10s", ...)` para alinear la tabla.
    - Para "mejor alumno": recorre la lista guardando el máximo en una variable.
    - Para la media de la clase: suma las medias de los alumnos que no tienen PENDIENTE.
...
**Objetivo:** Aplicar `Files.lines()`, y las operaciones `filter`, `map`, `limit`, `count`, `distinct`, `sorted` sobre un fichero de log real.
**Criterios:** CE5c · CE5d · CE5e
...
??? question "Pistas"
    ...
    - Para extraer el nivel: `linea.contains("[ERROR]")`, `linea.contains("[WARN ]")`, etc.
    - Para extraer el módulo de una línea de error: `linea.split("\\]")[2].trim().replace("[", "")`.
    - `Collectors.joining(", ", "[", "]")` para formatear la lista de módulos.
    - `stream.anyMatch(l -> l.contains("[ERROR]") && l.contains(palabra))` para `tieneErroresCriticos`.
...
**Objetivo:** Dominar `Collectors.groupingBy()`, `averagingDouble()`, `counting()`, `partitioningBy()`, `joining()` y `toMap()`.
**Criterios:** CE5b · CE5d · CE5e
...
??? question "Pistas"
    ...
    - `Collectors.groupingBy(Alumno::modulo, Collectors.averagingDouble(Alumno::media))`.
    - `Collectors.groupingBy(Alumno::modulo, Collectors.mapping(Alumno::nombre, Collectors.toList()))`.
    - `Collectors.partitioningBy(Alumno::aprobado, Collectors.counting())`.
    - Para el ranking: convierte el `Map` a un `Stream` de entradas, ordena por valor y muestra.
...
**Objetivo:** Dominar `ObjectOutputStream`, `ObjectInputStream`, `Serializable`, `serialVersionUID` y la persistencia de listas de objetos.
...
??? question "Pistas"
    ...
    - `try (ObjectOutputStream oos = new ObjectOutputStream(Files.newOutputStream(fichero))) { oos.writeObject(lista); }`.
    - En `cargar`, captura `InvalidClassException` por separado para dar un mensaje específico sobre versiones incompatibles.
    - Para `actualizarStock`: usa un bucle `for` sobre la lista buscando `p.getId() == id`, luego llama a `p.setStock(nuevoStock)`.
...
**Extensión opcional:** Añade un método `exportarCSV(List<Producto> productos, Path destino)` que exporte el inventario a CSV, y otro `importarCSV(Path origen)` que lo importe, permitiendo intercambio de datos con otras aplicaciones.
...
**Objetivo:** Dominar `Files.walk()`, combinar con `filter`, `map`, `sorted`, `collect` y obtener métricas de árboles de directorios.
...
??? question "Pistas"
    ...
    - Para la extensión: `String ext = p.getFileName().toString(); int dot = ext.lastIndexOf('.'); return dot > 0 ? ext.substring(dot) : "sin extensión";`.
    - Para agrupar por extensión: `Collectors.groupingBy(p -> obtenerExtension(p), Collectors.counting())`.
    - Para tamaño total: `.mapToLong(p -> { try { return Files.size(p); } catch (IOException e) { return 0; } }).sum()`.
    - Para los 5 más grandes: `.sorted(Comparator.comparingLong(...).reversed()).limit(5)`.
...
**Objetivo:** Integrar ficheros de texto, CSV, serialización, try-with-resources y API Stream en una aplicación cohesionada.

**Criterios:** CE5a · CE5b · CE5c · CE5d · CE5e
...
// 3. Cargar préstamos existentes (o lista vacía si no existen)
// 4. Realizar 2-3 préstamos de prueba
// 5. Mostrar libros disponibles
// 6. Devolver un libro
// 7. Mostrar estadísticas (libros más prestados, préstamos activos por socio)
// 8. Guardar el catálogo actualizado y los préstamos
// 9. Generar informe diario
```
...
|----------|-------------|------------|
| **Corrección funcional** | El programa produce la salida esperada | 40% |
| **Gestión de recursos** | Uso correcto de `try-with-resources` en todas las operaciones de I/O | 20% |
| **Calidad del código** | Legibilidad, organización, nombres descriptivos | 15% |
| **Manejo de errores** | Captura de excepciones adecuada, mensajes claros al usuario | 15% |
| **Uso de Stream API** | Uso apropiado de operaciones funcionales donde corresponde | 10% |

!!! tip "Consejo de entrega"
    Incluye en tu entrega un fichero `README.txt` con:
    - Descripción breve de lo que hace cada clase.
    - Cómo ejecutar el programa (ficheros de entrada necesarios).
